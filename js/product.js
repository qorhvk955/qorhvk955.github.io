document.addEventListener("DOMContentLoaded", () => {
  ...
  const productData = {
    sauce: [
      ...
      {
        title:
          "<span>저칼로리</span><br><span>머스타드</span> <span>소스</span>",
    ...
    dressing: [
      ...
      {
        title:
          "<span>저당</span> <span>사우전드</span> <span>아일랜드</span><br><span>드레싱</span>",
    ...
    food: [
      ...
      {
        title:
          "<span>저당</span> <span>브리또</span><br><span>양념치킨맛</span>",
      ...
      {
        title:
          "<span>저당</span> <span>브리또</span><br><span>숯불매콤맛</span>",
    ...
    drink: [
      ...
      {
        title:
          "<span>제로</span> <br> <span>샤인머스캣</span> <br> <span> 에이드</span>",
  ...
  function renderProducts(category) {
    ...
    // 기존 아이템 제거
    const existingItems = dragContainer.querySelectorAll(
      ".drag-item:not(.template)"
    );
    ...
    items.forEach((product, index) => {
      ...
      if (product.title.includes("브리또")) {
        clone.querySelector(".main-img").style.transform =
          "translateY(-50%) rotate(9deg)";
      }
  ...
  dragContainer.addEventListener("mouseup", () => {
    ...
    // 가장 가까운 아이템 위치 계산
    const items = [
      ...dragContainer.querySelectorAll(".drag-item:not(.template)"),
    ];
